...
const run = async () => {
  ...
  // Manager requires a managedTeam. canActForUser scopes manager by
  // reportingManager (not team), so any team works for this fixture.
  ...
  const team = await axios.post(`${BASE}/teams`, { name: `Smoke Timeblocks Team ${Date.now()}`, department: deptId }, adminAuth);
  const teamId = team.data.data.team._id;
  const manager = await createUser(adminAuth, "manager", { managedTeam: teamId });
  const member = await createUser(adminAuth, "member", { reportingManager: manager.userId });
  const otherManager = await createUser(adminAuth, "manager", { managedTeam: teamId });
